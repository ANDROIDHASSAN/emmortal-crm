...
const blank = {
  name: '', slug: '', shortDesc: '', longDesc: '', category: 'general', priceFrom: 0, featured: false, active: true,
  specs: { cell: '', bms: '', voltage: 0, ah: 0, sizeMm: '' },
...
export default function Products() {
  ...
  const openNew = () => { setF(blank); setModal({ new: true }); };
  const openEdit = (p) => {
    setF({
      ...blank, ...p,
      specs: { ...blank.specs, ...(p.specs || {}) },
      seo: { ...blank.seo, ...(p.seo || {}), keywords: (p.seo?.keywords || []).join(', ') },
  ...
  const save = async () => {
    const body = {
      name: f.name, slug: f.slug || undefined, shortDesc: f.shortDesc, longDesc: f.longDesc, category: f.category,
      priceFrom: Number(f.priceFrom), featured: !!f.featured, active: !!f.active,
      specs: { cell: f.specs.cell, bms: f.specs.bms, voltage: Number(f.specs.voltage), ah: Number(f.specs.ah), sizeMm: f.specs.sizeMm },
      seo: { title: f.seo.title, metaDescription: f.seo.metaDescription, keywords: (f.seo.keywords || '').split(',').map((s) => s.trim()).filter(Boolean) },
      images: f.imageUrl ? [{ url: f.imageUrl, alt: f.imageAlt || f.name }] : [],
    };
    try {
      if (modal.new) await createProduct(body).unwrap();
      else await updateProduct({ id: modal.id, ...body }).unwrap();
      toast.success('Product saved'); setModal(null);
  ...
  const cols = [
    ...
    { key: 'view', header: '', render: (r) => <a className="text-xs text-brand-700 hover:underline" href={`/products/${r.slug}`} target="_blank" rel="noreferrer">View ↗</a> },
  ...
  return (
    <div>
      <PageHeader title="Website / Products" subtitle="Storefront catalogue — each product is SEO-rendered & indexable"
        actions={<><a className="btn-ghost" href="/" target="_blank" rel="noreferrer">View site ↗</a><button className="btn-primary" onClick={openNew}>+ Product</button></>} />

      <DataTable columns={cols} rows={data?.data || []} loading={isFetching} emptyText="No products yet." />

      <Modal open={!!modal} onClose={() => setModal(null)} wide title={modal?.new ? 'New product' : 'Edit product'}
        ...
        <div className="mt-2 grid grid-cols-5 gap-3">
          ...
          <Field label="Size mm"><input className="input" value={f.specs.sizeMm} onChange={(e) => setF({ ...f, specs: { ...f.specs, sizeMm: e.target.value } })} /></Field>
        </div>
        <div className="mt-2 grid grid-cols-2 gap-4">
          <Field label="Image URL"><input className="input" value={f.imageUrl} onChange={(e) => setF({ ...f, imageUrl: e.target.value })} /></Field>
          <Field label="Image alt"><input className="input" value={f.imageAlt} onChange={(e) => setF({ ...f, imageAlt: e.target.value })} /></Field>
        </div>
        <div className="mt-2">
          <p className="label">SEO</p>
          <div className="grid grid-cols-2 gap-4">
            <Field label="Meta title"><input className="input" value={f.seo.title} onChange={(e) => setF({ ...f, seo: { ...f.seo, title: e.target.value } })} /></Field>
            <Field label="Keywords (comma sep)"><input className="input" value={f.seo.keywords} onChange={(e) => setF({ ...f, seo: { ...f.seo, keywords: e.target.value } })} /></Field>
          </div>
          <Field label="Meta description"><input className="input" value={f.seo.metaDescription} onChange={(e) => setF({ ...f, seo: { ...f.seo, metaDescription: e.target.value } })} /></Field>
        </div>
