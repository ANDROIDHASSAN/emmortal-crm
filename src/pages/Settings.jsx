...
export default function Settings() {
  ...
  const runBackup = async () => {
    setBackupBusy(true);
    try { const r = await fetch(`${API_BASE}/backup/run`, { method: 'POST', credentials: 'include' }); const j = await r.json(); if (r.ok) toast.success(`Backup done: ${j.data.documents} docs`); else toast.error(j.error?.message || 'Backup failed'); }
  ...
  return (
    <div>
      ...
      <SectionCard title="Users & roles"><DataTable columns={cols} rows={users?.data || []} emptyText="No users." /></SectionCard>
      <SectionCard title="Database backup">
        <p className="mb-3 text-sm text-slate-500">A monthly backup is emailed automatically. Run one now (gzipped JSON export of all collections).</p>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.new ? 'New user' : 'Edit user'}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Role"><select className="input" value={f.role} onChange={(e) => setF({ ...f, role: e.target.value })}>{ROLES.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}</select></Field>
