...
export default function Production() {
  ...
  const board = data?.data; const counts = board?.counts || {};
  const save = async () => { try { await createJob({ ...job, productionDate: date }).unwrap(); toast.success('Job added'); setModal(false); setJob({ title: '', voltage: 48, ah: 30, qty: 1 }); } catch (e) { toast.error(apiError(e)); } };
  const patch = async (id, body) => { try { await updateJob({ id, ...body }).unwrap(); } catch (e) { toast.error(apiError(e)); } };
  const sendComment = async () => { await patch(commentFor, { comment }); setComment(''); setCommentFor(null); toast.success('Comment added'); };

  return (
    <div>
      <PageHeader title="Production Planning" subtitle="A fresh board every day"
        actions={<div className="flex items-center gap-2"><input className="input" type="date" value={date} onChange={(e) => setDate(e.target.value)} /><button className="btn-primary" onClick={() => setModal(true)}>+ Add job</button></div>} />
